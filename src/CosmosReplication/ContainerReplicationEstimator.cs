    ...
    ContainerReplicationConfiguration config) : IContainerReplicationEstimator
{
            ...
            if (!Initialized)
            {
                try
                {
                    ...
                    var changeFeedEstimator = sourceContainer
                        .GetChangeFeedEstimatorBuilder(replicationName, ReportEstimatedPendingChanges, TimeSpan.FromSeconds(60))
    ...
    private Task ReportEstimatedPendingChanges(long estimatedPendingChanges, CancellationToken cancellationToken)
    {
