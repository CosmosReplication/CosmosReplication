...
public sealed class Worker : IHostedService, IDisposable
{
  ...
  public async Task StartAsync(CancellationToken cancellationToken)
  {
    ...
    var podName = Environment.GetEnvironmentVariable("POD_NAME") ?? Environment.GetEnvironmentVariable("HOSTNAME") ?? Environment.MachineName;
    _logger.LogInformation("Pod Name: {PodName}, Pod Namespace: {PodNamespace}", podName, podNamespace);
    ...
    var leaseLock = new LeaseLock(_client, podNamespace, leaseName, podName);
    ...
    await _leaderElector.RunAndTryToHoldLeadershipForeverAsync(cancellationToken);
    _logger.LeaderElectionStarted(podNamespace, podName);
  }

  public Task StopAsync(CancellationToken cancellationToken) => _replicationEstimatorService.StopAsync();
